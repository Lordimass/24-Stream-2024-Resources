...
def scrape(campaign_name):
    ...
    results = results.find("div", class_="page-campaign-layout_campaignPageLayout__ZK9_s")
    results = results.find("div", class_="t-campaign-page-template")
    results = results.find("main", class_="campaign-page_campaign__SFW7x t-campaign-page-template-content hrt-global-wrapper")
    results = results.find("div", class_="p-campaign")
    results = results.find("div", class_="p-campaign-sidebar")
    results = results.find("aside", class_="campaign-sidebar_sidebar__2TEAT o-campaign-sidebar")
    results = results.find("div", class_="campaign-sidebar_wrapper__Ay4Wk")
    results = results.find("div", class_="")
    results = results.find("div", class_="progress-meter_progressMeter__BYOlt")
    results = results.find("div", class_="progress-meter_progressMeterHeading__A6Slt")
